...
public class DCCommandListener implements CommandExecutor, TabCompleter {
	...
	public static String[] subCommands = new String[] {"subtypes", "nopickup", "blocklock", "noarmor", "drop"};
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (label.toLowerCase().startsWith("d")) {
			if (args.length == 0) { // He needs help!
				...
				if (isConsole || TFM_SuperadminList.isUserSuperadmin(sender)) {
					if (plugin.disguiseDB.containsKey(player.getName())) {
			...
			*/	
			...
			if (!isConsole && args.length > 0) {
				if (player.hasPermission("disguisecraft.other.undisguise")) {
					if (args[0].toLowerCase().equalsIgnoreCase("r") || args[0].toLowerCase().equalsIgnoreCase("radius")) {
						if (args.length > 1) {
							try {
