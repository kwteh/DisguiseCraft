...
public enum DisguiseType {
	...
	public static void getDataWatchers(org.bukkit.World world) {
    	...
    	try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Could not access datawatchers!");
		}
    	...
    	if (mapField == null) {
    		DisguiseCraft.log.log(Level.SEVERE, "Could not find the DataWatcher Map");
    	}
    	if (boolFields.isEmpty()) {
    		DisguiseCraft.log.log(Level.SEVERE, "Could not find any DataWatcher booleans!");
    	}
	}
	...
	public boolean isSubclass(Class<?> cls) {
		try {
			...
			DisguiseCraft.log.log(Level.WARNING, "Can't find entity class for the DisguiseType: " + name(), e);
		}
		return false;
	}
	...
	public DataWatcher newMetadata() {
		if (modelData.containsKey(id)) {
			...
			try {
				...
				DisguiseCraft.log.log(Level.SEVERE, "Could not construct a new DataWatcher", e);
				return null;
			}
		}
	}
	...
	public static DataWatcher copyDataWatcher(DataWatcher dw) {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Could not clone map in a datawatcher!", e);
		}
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Could not clone boolean in a datawatcher!", e);
		}
	...
	private static WatchableObject copyWatchable(WatchableObject watchable) {
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Could not copy a WatchableObject", e);
			return null;
		}
	}
	...
	public static String subTypes = "baby, black, blue, brown, cyan, " +
		...
		"silver, white, yellow, sheared, charged, tiny, small, big, " +
