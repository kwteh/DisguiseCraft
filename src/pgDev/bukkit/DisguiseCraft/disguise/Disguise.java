...
public class Disguise {
	...
	void mAdd(int index, Object value) {
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Could not add metadata to DataWatcher for a " + type.name() + " disguise", e);
		}
	}
	...
	void mWatch(int index, Object value) {
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Could not edit metadata in DataWatcher for a " + type.name() + " disguise", e);
		}
	}
	...
	public void safeAddData(int index, Object value, boolean forcemodify) {
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Something bad happened in a " + type.name() + " disguise!");
		} catch (IllegalAccessException e) {
			DisguiseCraft.log.log(Level.SEVERE, "Could not access the metadata map for a " + type.name() + " disguise!");
		}
	}
	...
	public void handleData() {
		if (!data.isEmpty()) {
			...
			try {
				...
				DisguiseCraft.log.log(Level.WARNING, "Bad cube size values in configuration!", e);
			}
			...
			for (String one : data) {
				if (one.startsWith("blockID:")) {
					...
					try {
						...
						DisguiseCraft.log.log(Level.WARNING, "Could not parse the integer of a disguise's block!");
					}
				}
			}
		}
			...
			for (String one : data) {
				if (one.startsWith("blockData:")) {
					...
					try {
						...
						DisguiseCraft.log.log(Level.WARNING, "Could not decode the byte of a disguise's block data!");
					}
				}
			}
		}
			...
			for (String one : data) {
				if (one.startsWith("cartType:")) {
					...
					try {
						...
						DisguiseCraft.log.log(Level.WARNING, "Could not decode the integer of a disguise's cart type!");
					}
				}
			}
		}
