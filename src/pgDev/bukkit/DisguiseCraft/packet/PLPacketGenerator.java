...
public class PLPacketGenerator extends DCPacketGenerator {
	...
	public PacketPlayOutSpawnEntityLiving getMobSpawnPacket(Location loc, String name) {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "PL: Unable to modify the integers for a " + d.type.name() +  " disguise!", e);
		}
		try {
			pC.getBytes().
				write(0, yp[0]).
			...
			DisguiseCraft.log.log(Level.SEVERE, "PL: Unable to modify the bytes for a " + d.type.name() +  " disguise!", e);
		}
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "PL: Unable to modify the metadata for a " + d.type.name() +  " disguise!", e);
		}
	...
	public PacketPlayOutNamedEntitySpawn getPlayerSpawnPacket(Location loc, short item) {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "PL: Unable to modify the integers for a player disguise!", e);
		}
		try {
			pC.getGameProfiles().
				write(0, gp);
		} catch (FieldAccessException e) {
			DisguiseCraft.log.log(Level.SEVERE, "PL: Unable to modify the GameProfile for a player disguise!", e);
		}
		try {
			pC.getBytes().
				write(0, yp[0]).
			...
			DisguiseCraft.log.log(Level.SEVERE, "PL: Unable to modify the bytes for a player disguise!", e);
		}
		try {
			pC.getDataWatcherModifier().
				write(0, new WrappedDataWatcher(d.metadata));
		} catch (FieldAccessException e) {
			DisguiseCraft.log.log(Level.SEVERE, "PL: Unable to modify the metadata for a player disguise!", e);
		}
	...
	public PacketPlayOutEntityDestroy getEntityDestroyPacket() {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "PL: Unable to modify the integer array for a destroy packet!", e);
		}
