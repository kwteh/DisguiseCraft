...
public class DCPacketGenerator {
	...
	public PacketPlayOutSpawnEntityLiving getMobSpawnPacket(Location loc, String name) {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Unable to set a field for a " + d.type.name() +  " disguise!", e);
		}
		return packet;
	}
	...
	public PacketPlayOutNamedEntitySpawn getPlayerSpawnPacket(Location loc, short item) {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Unable to set a field for a player disguise!", e);
		}
		return packet;
	}
	...
	public byte[] getYawPitch(Location loc) {
		...
		if (d.type.isVehicle()) { // Vehicle fix
			yaw = (byte) (yaw - 64);
		}
	...
	public PacketPlayOutEntityHeadRotation getHeadRotatePacket(Location loc) {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Unable to set a field for a head rotation packet!", e);
		}
	...
	public PacketPlayOutAnimation getAnimationPacket(int animation) {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Unable to set a field for an animation packet!", e);
		}
	...
	public PacketPlayOutEntityStatus getStatusPacket(int status) {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Unable to set a field for an entity status packet!", e);
		}
	...
	public PacketPlayOutEntityVelocity getVelocityPacket(int x, int y, int z) {
		...
		try {
			...
			DisguiseCraft.log.log(Level.SEVERE, "Unable to set a field for a velocity packet!", e);
		}
